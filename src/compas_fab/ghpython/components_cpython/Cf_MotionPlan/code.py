...
assembled plan plus composite outputs for visualisation.

`planes` is a DataTree with one branch per trajectory step (per-step
...
class MotionPlanComponent(Grasshopper.Kernel.GH_ScriptInstance):
    def RunScript(self, name: str, start_state, steps, robot_cell, group: str, description: str):
        ...
        cell_states = list(plan.iter_cell_states())

        if robot_cell is None:
            return (plan, [], None, cell_states, plan.duration, plan.end_state)
        ...
        return (plan, planes_tree, polylines_per_step or None, cell_states, plan.duration, plan.end_state)
