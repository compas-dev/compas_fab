...
class PyBulletClient(PyBulletBase, ClientInterface):
    ...
    def check_collisions(self, configuration=None, verbose=False, full_report=False):
        ...
        verbose : :obj:`bool`
            When ``True``, additional information is printed. Note that this significantly
            reduces performance. Defaults to ``False``.
        ...
        for link_name, link_id in self.robot_link_puids.items():
            for tool_name, tool_id in self.tools_puids.items():
                tool_state = self.robot_cell_state.tool_states[tool_name]
                ...
                try:
                    self._check_collision(
                        self.robot_puid, "robot_" + link_name, tool_id, tool_name, link_index_1=link_id
                    )
                    verbose_print(cc_pair_info + " - PASS")
        ...
        for link_name, link_id in self.robot_link_puids.items():
            for body_name, body_ids in self.rigid_bodies_puids.items():
                rigid_body_state = self.robot_cell_state.rigid_body_states[body_name]
                ...
                for body_id in body_ids:
                    try:
                        self._check_collision(self.robot_puid, "robot_" + link_name, body_id, body_name, link_id)
        ...
        for body_name, body_ids in self.rigid_bodies_puids.items():
            ...
            # Skip over hidden bodies
            if self.robot_cell_state.rigid_body_states[body_name].is_hidden:
                verbose_print(cc_pair_info + " - SKIPPED (HIDDEN)")
                continue
            # Skip over non-attached bodies because there is no need to check between two static bodies
            if not (
                self.robot_cell_state.rigid_body_states[body_name].attached_to_tool
                or self.robot_cell_state.rigid_body_states[body_name].attached_to_link
            ):
                verbose_print(cc_pair_info + " - SKIPPED (NOT ATTACHED TO LINK)")
                continue
            for other_body_name, other_body_ids in self.rigid_bodies_puids.items():
                # Skip over hidden bodies
                if self.robot_cell_state.rigid_body_states[other_body_name].is_hidden:
                    continue
                # Skip over same body pairs
                if body_name == other_body_name:
                    continue
                # Skip over allowed touch bodies
                if body_name in self.robot_cell_state.rigid_body_states[other_body_name].touch_bodies:
                    verbose_print(cc_pair_info + " - SKIPPED (ALLOWED TOUCH BODY)")
                    continue
                if other_body_name in self.robot_cell_state.rigid_body_states[body_name].touch_bodies:
                    verbose_print(cc_pair_info + " - SKIPPED (ALLOWED TOUCH BODY)")
                    continue
                # Perform collision check
                for body_id in body_ids:
                    for other_body_id in other_body_ids:
                        ...
                        try:
                            self._check_collision(body_id, body_name, other_body_id, other_body_name)
                            verbose_print(cc_pair_info + " - PASS")
                        except CollisionCheckInCollisionError:
                            verbose_print(cc_pair_info + " - COLLISION")
                            collision_messages.append(cc_pair_info + " - COLLISION")
        ...
        for tool_name, tool_id in self.tools_puids.items():
            ...
            for body_name, body_ids in self.rigid_bodies_puids.items():
                ...
                for body_id in body_ids:
                    try:
                        self._check_collision(tool_id, tool_name, body_id, body_name)
                        verbose_print(cc_pair_info + " body_id '{}' - PASS".format(body_id))
                    except CollisionCheckInCollisionError:
                        verbose_print(cc_pair_info + " body_id '{}' - COLLISION".format(body_id))
                        collision_messages.append(cc_pair_info + " body_id '{}' - COLLISION".format(body_id))
