""" ik util functions
These utility functions are modified from `ss-pybullet's ikfast utils <https://github.com/caelan/ss-pybullet/tree/master/pybullet_tools/ikfast>`.

The reason for this adaption is to give `compas_fab` users more control on using IKfast,
For now, these functions take only pybullet-based parameters, e.g. Pose = (point, quat).
"""
...
from pybullet_planning import multiply, get_link_pose, get_movable_joints, \
