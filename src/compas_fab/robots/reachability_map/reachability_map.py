...
class ReachabilityMap(Data):
    ...
    Attributes
    ----------
    frames : list of list of [`Frame`][compas.geometry.Frame]
        The frames at which the IK solutions are calculated.
    configurations : list of list of list of [`Configuration`][compas_robots.Configuration]
        The configurations at the frames.
    score : list of int
        The number of solutions per frame list (2D)
    points : list of [`Point`][compas.geometry.Point]
    ...
    Notes
    -----
    See Also [reuleaux](https://wiki.ros.org/reuleaux)
    """
    ...
    def calculate(
        ...
        ----------
        frame_generator : generator
            A 2D frame generator to yield [`Frame`][compas.geometry.Frame].
        planner : [`PlanningInterface`][compas_fab.backends.PlanningInterface]
