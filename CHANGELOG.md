...
* Redesigned the Grasshopper component library (CPython / Rhino 8) for the new stateless planning model. The legacy IronPython `components/` set was retired; all components now live under `src/compas_fab/ghpython/components_cpython/` and target Rhino 8 CPython exclusively. New categories under "COMPAS FAB":
  * *Backends* — `Cf_RosClient` (now exposes an optional `transport` input for per-connection roslibpy transport selection — `twisted`/`asyncio`/`cli` — falling through to `ROSLIBPY_TRANSPORT` and `roslibpy.set_default_transport` when empty; the underlying `RosClient` class gained the same `transport=` constructor kwarg), `Cf_MoveItPlanner`, `Cf_AnalyticalKinematicsPlanner`.
  ...
  * *Planning* — `Cf_ForwardKinematics`, `Cf_InverseKinematics` (accepts a bare `compas.geometry.Frame` or Rhino `Plane` as `target` and auto-wraps it as `FrameTarget(target_mode=ROBOT)`; defaults `start_state` to a zero-config state derived from `planner.robot_cell` when unwired; warns when `planner` or `target` are wired but received None; emits both the `configuration` *and* an updated `cell_state` so `Visualize` / `ForwardKinematics` / the next planner step can wire straight in without a `Cf_SetRobotConfiguration` step in between), `Cf_PlanMotion` and `Cf_PlanCartesianMotion` (both gain `planes` and `polyline` outputs — `planes` is the list of EE Rhino planes obtained by running FK on each trajectory point through the same cell+state, `polyline` connects their origins; powered by a new `compas_fab.ghpython.trajectory_to_planes_and_polyline` helper that uses `RobotCell.forward_kinematics_target_frame` in-process so a 30-point trajectory resolves in low single-digit ms instead of a ~600 ms round-trip per `compute`. Planning failures (no plan found, start/goal in collision, timed out, …) now flag the component red with the backend error message; the dedicated `error` output was dropped because the canvas already shows it on the node), `Cf_DeconstructTrajectory` (expands a JointTrajectory into a list of `Configurations` plus, when `start_state` is wired, a list of `RobotCellStates` — pipe an index slider into the second output to scrub a trajectory through `VisualizeRobotCell`).
...
* `MoveItPlanMotion.plan_motion` and `MoveItPlanCartesianMotion.plan_cartesian_motion` were re-raising every typed planner error (`MPNoPlanFoundError`, `MPStartStateInCollisionError`, `MPTargetInCollisionError`, `MPSearchTimeOutError`, `PlanningGroupNotExistsError`, …) wrapped in `RosValidationError`, so callers catching `MotionPlanningError` as the docstring promises were missing every real failure. Both methods now unwrap `RosValidationError` and re-raise `e.original_exception`, matching the pattern already used by `MoveItInverseKinematics.iter_inverse_kinematics`. `Cf_PlanMotion` / `Cf_PlanCartesianMotion` now catch `BackendError` (as a backstop for `RosError` and `PlanningGroupNotExistsError`) in addition to `MotionPlanningError`.
* `JointTrajectoryPoint.joint_names` was empty on points produced by the MoveIt and analytical Cartesian planners because the ROS message spec only carries names on the parent `JointTrajectory` (and the analytical path simply forgot). Both backends now attach the trajectory's `joint_names` to each `JointTrajectoryPoint` at construction, matching the PyBullet planner. Without this, any consumer that walks the points by name (e.g. the new `Cf_PlanMotion` / `Cf_PlanCartesianMotion` planes/polyline visualization, or `Cf_DeconstructTrajectory`) was silently no-op-ing and showing FK of the start state for every point. The viz helper and `Cf_DeconstructTrajectory` now use `Configuration.merged()` (with a fallback to the parent trajectory's `joint_names`, so the visualization still works against older MoveIt trajectories that don't carry the per-point names).
