...
* `Load Robot Cell From Library` and `Load Robot Cell From ROS` now auto-create a Boolean Toggle on their `load_geometry` input (via the new `ensure_boolean_toggle` helper in `compas_fab.ghpython`) so geometry loading can be flipped on/off with one click.
...
* `Load Robot Cell From ROS` replaced its `load` input with a `reload` input matching `Load Robot Cell From Library`. The cell is now fetched automatically on the first run (when nothing is cached) instead of requiring the user to flip `load` to `True`; `reload` forces a refetch from ROS. Previously a freshly-dropped component with a connected client silently returned `None` until `load` was toggled on.
* `Load Robot Cell From ROS` now outputs the `RosClient` as its first (pass-through) output and records the fully-registered cell on it (`ros_client.robot_cell`), and `MoveIt Planner` dropped its `robot_cell` input in favour of reading the cell from `ros_client.robot_cell`. Wiring the Load component's `ros_client` output into the planner is now enough to both supply the cell (with any registered tools / rigid bodies) and guarantee it is loaded before the planner runs.
