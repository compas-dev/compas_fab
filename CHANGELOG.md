...
* Redesigned the Grasshopper component library (CPython / Rhino 8) for the new stateless planning model. The legacy IronPython `components/` set was retired; all components now live under `src/compas_fab/ghpython/components_cpython/` and target Rhino 8 CPython exclusively. New categories under "COMPAS FAB":
  * *Backends* — `Cf_RosClient` (with a `transport` input — `twisted`/`asyncio`/`cli` — wired through to a new `RosClient(transport=…)` constructor kwarg; falls back to roslibpy's process-wide default when empty), `Cf_MoveItPlanner`, `Cf_AnalyticalKinematicsPlanner`.
...
* Added `Cf_TrajectoryStep`, `Cf_StateChangeStep`, and `Cf_MotionPlan` Grasshopper components under *Planning*. The two step builders wrap a `JointTrajectory` (or a `RobotCellState` for a discrete state change) into a named `PlanStep` next to the data that produced it — names cannot drift out of sync with the underlying object. `Cf_MotionPlan` takes the ordered `steps` list, an optional `robot_cell` (for signature verification on load), and emits the assembled `plan` plus composite visualisation outputs: a DataTree of EE planes with one branch per trajectory step, a parallel list of polylines (one per step), `duration` and `end_state`. The viz reuses `trajectory_to_planes_and_polyline` per step, walking the plan's state chain so each segment's FK uses the correct pre-state. The helper's signature was relaxed to take `robot_cell` directly (instead of digging through `planner.client.robot_cell`), making it reusable from any caller that already has a cell.
* Added `compas_fab.robots.MotionPlan` and `compas_fab.robots.PlanStep` — a higher-level assembly layer that threads a single `RobotCellState` through a chain of trajectory and state-change steps. Each trajectory step derives its post-state from the predecessor (last-point applied); state-change steps (gripper toggle, tool attach/detach) carry an explicit post-state. The canonical use case is pick-and-place: `MotionPlan(name, start_state).append_trajectory(...).append_state_change(...)...`, fluent chaining. Lookup is by name (`plan.step_by_name(...)`) rather than index, so a future tree extension won't force an API break. Optional `robot_cell` constructor argument stores a structural signature (robot name + tool/body ids, SHA-256) so `plan.verify_cell(cell)` fails loudly when a plan is loaded against the wrong cell. Serialisation strips the redundant `start_state` from each embedded trajectory (the plan owns the chain) and omits the derived `post_state` of trajectory steps; only the plan's `start_state` and explicit state-change post-states are stored. Planners' return types are unchanged — `MotionPlan` is purely an additive composition layer.
