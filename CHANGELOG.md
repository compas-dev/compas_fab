...
* The `Inverse Kinematics` Grasshopper component now augments an IK-failure message with a collision check on the start state (when the planner supports it): if the start state is in collision the colliding pairs are listed, explaining why no collision-free solution exists; planners without collision checking are handled gracefully.
