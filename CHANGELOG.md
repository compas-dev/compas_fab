...
* Eliminated intermittent CI failures in `integration.yml` on the `RosClient` doctest (`>>> with RosClient() as client: ...`) that raised `RosTimeoutError('Failed to connect to ROS')` when `--doctest-modules` started before rosbridge inside the docker stack was accepting websocket connections. Added a `Wait for rosbridge endpoints` step that TCP-probes `localhost:9090` (ROS 1) and `localhost:9091` (ROS 2) for up to 120s before pytest runs, so the first ROS-touching test no longer races the container's cold-start.
